...
function init() { 
  ...
  const addTouchAction = block =>{
    ...
    const onGrab = e =>{
      ...
      settings.grabInterval = setInterval(()=> {
        ...
        block.x = mousePos.x - left
        block.y = mousePos.y - top
        setStyles(block)
  ...
  const animateBlocks = () => {
    settings.shapes.forEach(shape => { 
      ...
      // animateBlock(grabPoint)
    })
  }


  const createBlocks = offset => {
    const newShape = {
      blocks: [],
      lines: [],
    ...
    grabPoint.velocity = grabPoint.create(0, 0)
      grabPoint.acceleration = grabPoint.create(0, settings.gravity)  
      grabPoint.accelerate = function(acceleration) {
        this.velocity.addTo(acceleration)
      }
  
    elements.machine.append(grabPoint.el)
    setStyles(grabPoint)
  
    newShape.blocks.forEach(block => {
      block.grabPoint = grabPoint
    ...
    ;[
      {
        ...
        end: newShape.blocks[1],
      },
      {
        start: newShape.blocks[1],
        end: newShape.blocks[4],
      },
      {
        start: newShape.blocks[4],
        end: newShape.blocks[3],
      },
      {
        start: newShape.blocks[3],
        end: newShape.blocks[0],
      },
      {
        start: newShape.blocks[0],
        end: newShape.blocks[2],
      },
      {
        start: newShape.blocks[1],
        end: newShape.blocks[2],
